...
def agregar_tutorial(request):
    ...
    Cancion.objects.create(titulo=titulo,afinacion=afinacion,link_youtube=link_youtube)
...
def editar_tutorial(request):
    ...
    afinacion = body['afinacion']
    link_youtube = body['link_youtube']
    Cancion.objects.filter(id=id_tutorial).update(titulo=titulo,afinacion=afinacion,link_youtube=link_youtube)
...
def borrar_captura(request):
    ...
    fullpath = os.getcwd() + '/app/static/uploads/' + nombre
    os.remove(fullpath)
